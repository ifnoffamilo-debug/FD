...
except ImportError:  # Ручной учёт работает даже без SDK
...
class AIService:
    ...
    def __init__(self, api_key: str | None, model: str, base_url: str) -> None:
        ...
        if api_key and AsyncOpenAI is None:
            raise RuntimeError("Для подключения Groq установите зависимости: pip install -r requirements.txt")
        self.client = (
    ...
    async def parse_operation(self, text: str) -> ParsedOperation:
        ...
        try:
            try:
                ...
                # Некоторые модели Groq не поддерживают JSON mode — повторяем обычный запрос.
                response = await self.client.chat.completions.create(
                    model=self.model,
                    messages=messages,
                    temperature=0,
    ...
    async def answer_finance_question(self, question: str, context: str) -> str:
        if not self.client:
            return (
                "Groq пока не подключён. Добавьте AI_API_KEY в переменные окружения "
